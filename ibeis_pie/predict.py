...
def predict(img_path, config=None, config_path=None, display=False, output_dir=None, gtruth_path=None, skip_illustration=True):
    #Check input parameters
    if not os.path.isfile(img_path):
        raise ValueError('Image file does not exist or not a file. Check input. Current input: ', img_path)

    if config is None and not os.path.isfile(config_path):
        raise Exception('Config not provided. Config file does not exist or not a file. Check input. Current input: ', config_path)
    ...
    if display:
        ...
        if respond == 'save' and md.done:
            ...
            croppedpath = crop_im_by_mask(img_path, maskpath, cropped_dir, padding = 0, square=square)
    ...
    export_emb(embedding, info=[[img_path], [img_path], [img_path]], folder=folder, prefix=emb_prefix, info_header=['label,filename,name'])
    ...
    if gtruth_exists:
        ...
        if true_lab in pred_lbl:
            ...
            fig.suptitle('Correct match is in top-{}.\nGround truth info is provided in {}'.format(match_num, gtruth_path), fontsize=16)
        else:
            fig.suptitle('No match in top-10 predictions.\nGround truth is provided in {}'.format(gtruth_path), fontsize=16)
    else:
...
if __name__ == "__main__":
    ...
    gtruth = args.gtruth
    predict_illustrate(img_path, config_path, display, output_dir, gtruth, skip_illustration=False)
