...
class TripletLossPoseInv(BaseModel):
    def __init__(
        ...
        # Define main branch
        super(TripletLossPoseInv, self).__init__(
            input_shape,
            backend,
            frontend,
            embedding_size,
    ...
    def compile_model(self, lr, margin=0.5, weights=[1.0, 1.0], loss_func=None):
        ...
        if loss_func == 'triplet_loss_mult':
            self.model.compile(
                loss=lambda y_true, y_pred: triplet_loss_mult(
                    y_true, y_pred, margin, n_poses=self.n_poses, n_imgs=self.bs
                ),
                optimizer=optimizer,
            )
            ...
            self.model.compile(
                loss=lambda y_true, y_pred: pose_variance(
                    y_true, y_pred, n_poses=self.n_poses, n_imgs=self.bs
                ),
                optimizer=optimizer,
            )
            ...
            self.model.compile(
                loss=lambda y_true, y_pred: triplet_pose_loss(
                    y_true, y_pred, margin, weights, n_poses=self.n_poses, n_imgs=self.bs
                ),
                optimizer=optimizer,
            )
            print(
                'Model is compiled with triplet_pose_loss = triplet_loss_mult + pose_invariant loss, weights {}'.format(
                    weights
                )
            )
            ...
            self.model.compile(
                loss=lambda y_true, y_pred: semihard_pose_loss(
                    y_true, y_pred, margin, weights, n_poses=self.n_poses, n_imgs=self.bs
                ),
                optimizer=optimizer,
            )
            print(
                'Model is compiled with semihard_pose_loss: semihard over the whole batch and pose invariant term'
            )
        else:
            raise Exception(
                'Only semi_hard_triplet, triplet_loss_mult and triplet_pose_loss are supported'
            )
