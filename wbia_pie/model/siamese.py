...
class Siamese(BaseModel):
    ...
    def siamese_block(self):
        ...
        out = Lambda(euclidean_distance, output_shape=eucl_dist_output_shape)(
            [out_a, out_b]
        )
    ...
    def compile_model(self, learning_rate, loss_func=None):
        ...
        if self.loss_func == 'binary_crossentropy':
            ...
            self.model.compile(
                loss=contrastive_loss, optimizer=optimizer, metrics=['accuracy']
            )
        else:
            raise Exception(
                'Only binary_crossentropy or contrastive losses are supported for Siamese network'
            )
    ...
    def preproc_predict(self, imgs, batch_size=32):
        ...
        imgs_preds = np.zeros(
            (imgs.shape[0],) + self.top_model.get_output_shape_at(0)[1:]
        )
        ...
        for i in range(images.shape[0]):
            for j in range(i):
                ...
                distances[idx] = self.model.predict_on_batch(
                    [np.expand_dims(img_1, 0), np.expand_dims(img_2, 0)]
                )
