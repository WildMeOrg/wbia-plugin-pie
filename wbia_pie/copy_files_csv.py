...
argparser.add_argument(
    ...
    '--trg',
    required=True,
    help='Target folder to copy images to. The folder will be created if  not exists',
)
argparser.add_argument(
    '-f',
    '--file',
    required=True,
    help='Csv file with a list of files in the first column to copy (file with header)',
)
...
def _main_(args):
    ...
    labels = np.genfromtxt(
        labels_file, dtype=str, skip_header=1, delimiter=',', usecols=(0)
    )
