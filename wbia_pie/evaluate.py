...
argparser.add_argument(
    '-c', '--conf', help='path to configuration file', default='config.json'
)
argparser.add_argument(
    '-s', '--split_num', help='split number for K-fold', type=int, default=-1
)
argparser.add_argument(
    '-m', '--mode', help='evaluation mode: all, pairs or life', default='all'
)
...
def evaluate(config_path, split_num=-1, mode='all'):
    ...
    print(
        'Date: {} / EVALUATION of experiment id: {}'.format(
            datetime.now(), config['train']['exp_id']
        )
    )
    ...
    if mode in ('all', 'compute'):
        ...
        export_emb(
            train_preds,
            info=[train_labels, files_train, train_names],
            folder=folder,
            prefix='train',
        ...
        export_emb(
            test_preds,
            info=[test_labels, files_test, test_names],
            folder=folder,
            prefix='test',
            info_header=['class,file,name'],
        )
