...
def triplet_pose_loss(
    y_true, y_preds, margin=0.5, weights=[1.0, 0.01], n_poses=4, n_imgs=40
):
    ...
    total_loss = tf.add(
        weights[0] * triplet_loss, weights[1] * pose_inv_loss, 'triplet_pose_loss'
    )
    return total_loss


def semihard_pose_loss(
    y_true, y_preds, margin=0.5, weights=[1.0, 0.01], n_poses=4, n_imgs=40
):
